...
class SimpleYOLOWorldDetector(YOLODetector):
    ...
    def loss(self, batch_inputs: Tensor,
        ...
        if self.reparameterized:
            ...
            # Create txt_masks (all ones since we use fixed embeddings)
            batch_size = img_feats[0].shape[0]
            num_classes = txt_feats.shape[1]
            txt_masks = txt_feats.new_ones(batch_size, num_classes)
            losses = self.bbox_head.loss(img_feats, txt_feats,
                                         txt_masks, batch_data_samples)
        return losses
