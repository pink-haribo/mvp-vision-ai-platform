...
class UltralyticsAdapter(TrainingAdapter):
    ...
    def prepare_dataset(self):
        ...
        if converted_from_dice:
            # Use the data.yaml generated by DICE conversion
            self.data_yaml = os.path.join(dataset_path, "data.yaml")
            print(f"[prepare_dataset] Using data.yaml from DICE conversion: {self.data_yaml}")
            sys.stdout.flush()
        else:
    ...
    def _auto_split_dataset(self, images_base: Path, train_folder: str, split_ratio: float = 0.8) -> Tuple[str, str]:
        ...
        if not source_dir.exists():
            print(f"[_auto_split_dataset] Source dir {source_dir} not found, using base: {images_base}")
    ...
    def train(self, start_epoch: int = 0, checkpoint_path: str = None, resume_training: bool = False) -> List[MetricsResult]:
        ...
        from platform_sdk import TrainingCallbacks
